...
export default function ScopeVersionsPageClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [cfgRes, listRes, assetTypesRes, departmentsRes, locationsRes] = await Promise.all([
  ...
  async function createScopeVersion() {
    ...
    try {
      ...
      if (!createdId) {
        router.push("/governance/scope");
        return;
      }

      router.push(`/governance/scope/${createdId}`);
    } catch (error) {
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-[1fr_1fr]">
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="rounded-md border bg-gray-50 p-3">
                <div className="text-sm font-semibold text-gray-900">Asset Types in Scope</div>
                <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
                  {assetTypes.map((row) => (
                    <label key={row.code} className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
                        checked={selectedAssetTypes.includes(row.code)}
                        onChange={() =>
                  ...
                  {assetTypes.length === 0 ? (
                    <div className="text-sm text-gray-500">No asset types available.</div>
                  ) : null}
                </div>
              </div>

              <div className="rounded-md border bg-gray-50 p-3">
                <div className="text-sm font-semibold text-gray-900">Departments in Scope</div>
                <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
                  {departments.map((row) => (
                    <label key={row.id} className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
                        checked={selectedDepartments.includes(row.id)}
                        onChange={() =>
              ...
              <div className="rounded-md border bg-gray-50 p-3">
                <div className="text-sm font-semibold text-gray-900">Locations in Scope</div>
                <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
                  {locations.map((row) => (
                    <label key={row.id} className="flex items-center gap-2 text-sm text-gray-700">
                      <input
              ...
              <div className="rounded-md border bg-gray-50 p-3">
                <div className="text-sm font-semibold text-gray-900">Environments</div>
                <div className="mt-3 flex flex-wrap gap-4">
                  {ENV_OPTIONS.map((env) => (
                    <label key={env} className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex min-w-0 flex-1 items-center gap-3 overflow-x-auto whitespace-nowrap text-sm font-medium text-gray-600 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {STATUSES.map((s) => (
                  <Link
                    key={s}
                    href={buildHref({ status: s, page: 1, pageSize })}
            ...
            <div className="mt-4 rounded-md border bg-gray-50 p-3">
              <div className="text-sm font-semibold text-gray-900">scope_json Preview</div>
              <pre className="mt-2 max-h-56 overflow-auto rounded-md bg-white p-3 text-xs">
