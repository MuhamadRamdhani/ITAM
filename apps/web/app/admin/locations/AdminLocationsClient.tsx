...
export default function AdminLocationsClient() {
  ...
  const [data, setData] = useState<LocationsListData>({
    total: 0,
    items: [],
    page: 1,
    page_size: 10,
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      try {
        ...
        setErr(eAny?.message || "Failed to initialize locations page");
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadLocations() {
      ...
      try {
        ...
        const res = await apiGet<LocationsListData>(
          `/api/v1/admin/locations?${qs.toString()}`,
          {
            loadingKey: "locations_list",
            loadingDelay: 300,
          }
        );
        if (cancelled) return;
        ...
        setErr(eAny?.message || "Failed to load locations");
      } finally {
  ...
  async function reloadLocations(customPage = page) {
    const qs = new URLSearchParams();
    if (q.trim()) qs.set("q", q.trim());
  ...
  async function onCreateLocation(e: React.FormEvent) {
    ...
    try {
      await apiPostJson<{ location: LocationItem }>("/api/v1/admin/locations", {
      ...
      setCreateCode("");
      setCreateName("");
      setOk("Location berhasil dibuat.");

      setPage(1);
      setQ("");
      setQInput("");

      await reloadLocations(1);
    } catch (eAny: any) {
      if (eAny?.code === "LOCATION_CODE_TAKEN") {
        setErr("Code location sudah digunakan di tenant ini.");
      } else {
        setErr(eAny?.message || "Failed to create location");
      }
    } finally {
  ...
  async function onSaveEdit(e: React.FormEvent) {
    ...
    try {
      await apiPatchJson<{ location: LocationItem }>(
        `/api/v1/admin/locations/${editId}`,
        {
      ...
      setOk("Location berhasil diupdate.");
      ...
      if (eAny?.code === "LOCATION_CODE_TAKEN") {
        ...
        setErr(eAny?.message || "Failed to update location");
      }
    } finally {
