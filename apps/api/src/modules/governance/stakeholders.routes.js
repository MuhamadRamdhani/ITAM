...
export default async function governanceStakeholdersRoutes(fastify) {
  ...
  fastify.get("/", async function handler(request, reply) {
    try {
      const data = await listStakeholdersRegistersService(db, request, request.query);
  ...
  fastify.get("/:id", async function handler(request, reply) {
    try {
      const data = await getStakeholdersRegisterDetailService(db, request, request.params.id);
      reply.send({ ok: true, data });
    } catch (err) {
      sendError(reply, err);
    }
  ...
  fastify.post("/", async function handler(request, reply) {
    try {
      const data = await createStakeholdersRegisterService(db, request, request.body ?? {});
  ...
  fastify.patch("/:id", async function handler(request, reply) {
    try {
      const data = await updateStakeholdersRegisterService(
        db,
        request,
        request.params.id,
        request.body ?? {}
